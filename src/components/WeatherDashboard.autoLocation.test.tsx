...
describe('WeatherDashboard Auto Location', () => {
    ...
    it('should attempt auto-location when no cities are saved and hasAutoLocated is false', async () => {
        ...
        // Verify storage set for autoLocationFallback
        expect(storage.setAsync).toHaveBeenCalledWith('autoLocationFallback', expect.objectContaining({ city: 'New York' }));
    });

    it('should attempt auto-location with fallback if permission is denied', async () => {
        (storage.get as any).mockImplementation((key: string) => {
            if (key === 'savedCities') return Promise.resolve([]);
            if (key === 'autoLocationPermissionDenied') return Promise.resolve(true); // Denied previously
            if (key === 'autoLocationFallback') return Promise.resolve({
                 city: 'Old York', temperature: 15, condition: 'Cloudy',
                 humidity: 50, windSpeed: 5, feelsLike: 12, pressure: 1012,
                 visibility: 10, uvIndex: 5, hourlyForecast: [], dailyForecast: [],
                 lat: 40.71, lon: -74.00, source: 'openweathermap', isOffline: true
            });
            return Promise.resolve(null);
        });
        ...
        // Give it time to render the fallback card
        await screen.findByText('📍 Old York');

        // It shouldn't prompt again
